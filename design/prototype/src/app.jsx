  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <TweaksPanel>
        ...
        <TweakSelect label="Aesthetic" value={t.aesthetic}
          options={[{ value: 'document', label: 'Warm document' }, { value: 'neutral', label: 'Clean neutral' }, { value: 'technical', label: 'Quiet technical' }]}
          onChange={v => setTweak('aesthetic', v)} />
        <TweakRadio label="Theme" value={t.theme} options={['light', 'dark']} onChange={v => setTweak('theme', v)} />
        <TweakRadio label="Density" value={t.density} options={['roomy', 'balanced', 'compact']} onChange={v => setTweak('density', v)} />
        <TweakSelect label="Role palette" value={t.palette}
          options={[{ value: 'soft', label: 'Soft (default)' }, { value: 'vivid', label: 'Vivid' }, { value: 'earthen', label: 'Earthen' }]}
          onChange={v => setTweak('palette', v)} />
        <TweakSection label="Playback" />
        <TweakToggle label="Auto-play on load" value={t.autoplay} onChange={v => setTweak('autoplay', v)} />
        <TweakSlider label="Speed" value={t.speed} min={0.5} max={2.5} step={0.1} unit="×" onChange={v => setTweak('speed', v)} />
        <TweakButton label="Replay scene" onClick={scene.replay} />
      </TweaksPanel>
    </div>
  );
}
