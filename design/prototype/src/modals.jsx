...
function NewWorkbenchModal({ agents, onClose, onCreate }) {
  ...
  const allTemplates = TEMPLATES.concat(window.RT.userTemplates || []);
  const tpl = allTemplates.find((t) => t.id === sel);
  const roleColors = window.RT.ROLE_COLORS;
  return (
...
Object.assign(window, { Modal, Btn, Pipeline, NewWorkbenchModal, NewTaskModal, AddAgentModal });
